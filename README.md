...
1. **高波数では PDE 残差損失が学習を破綻させる。** kL ≈ 146 の広帯域
   ...
   学習し（PDE ~0.02）、学習ステップに対し単調改善（20k/40k/80k/160k で
   −4.13/−5.87/−7.10/−7.83 dB）して 80k で baseline（−6.18 dB）、160k で
   mMLP（−7.78 dB）を上回り **PDE 損失系の最良**となる（同一コマンドの
   再現実行でも −7.83 dB で、走行間分散は小さい）。PDE 損失で学習できた
   唯一の sine 系ネットワークである。なお sine 特徴層 + mMLP ボディの
   合成（KPlaneMMLP）は安定だが −5.31 dB 止まりで相乗しなかった。
