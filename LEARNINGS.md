...
- **[verified 2026-06-13]** KScaledSiren は学習ステップに対し単調改善する（20k/40k/80k/160k → −4.13/−5.87/−7.10/−7.83 dB、cosine lr）。80k で tanh baseline（−6.18）超え、160k で mmlp（−7.78）超え。ただしダブリングあたりの改善は −1.74 → −1.23 → −0.73 dB と鈍化（飽和傾向）。
  **Rule:** 安定化した PINN が頭打ちに見えても、cosine スケジュールの総ステップ延長をまず試す。ただし収穫逓減を見越して外挿する。
- **[verified 2026-06-13]** ksiren の走行間分散は小さい（同一コマンド 2 回で −7.83/−7.83 dB、終端 data loss は 0.0020 vs 0.0039 と異なるのに NMSE 一致）。tanh の双安定（+0.07 ↔ −6.18）とは対照的。
- **[verified 2026-06-13]** sine 平面波特徴層 + ModifiedMLP ボディの合成（KPlaneMMLP）は発散しないが −5.31 dB止まりで、純 sine ネット（ksiren）にも mmlp にも劣る。
  **Rule:** 曲率整合した sine 特徴の後段に tanh 系ボディを足しても相乗しない — sine 系は一貫して sine で構成する方がよい。
